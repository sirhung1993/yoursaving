'use strict'
module.exports = class UserIncome {
    /*
    {
        age: 20,
        monthSalary: 9000000,
  ...
  constructor (userIncome) {
    this.offsetMoney = parseInt(userIncome.offsetMoney)
...
*/
  ...
  setDefault () {
    this.offsetMoney = isNaN(this.offsetMoney) ? 0 : this.offsetMoney
...
*/
  ...
  ConvertToLetter (arrayOfTotalAsset) {
    ...
    if (this.typeOfCurrency === 'vnd' && typeof convertedToLetter === 'object') {
    ...
    } else if (typeof convertedToLetter === 'number') {
      convertedToLetter = parseInt(convertedToLetter / 1000000) +
              ' triệu ' + parseInt(convertedToLetter % 1000000) + ' ngàn Đồng'
    } else {
      return Promise.reject(
  ...
  ReadableEstimationSaving (EstimationSaving) {
    ...
    } else if (typeof EstimationSaving === 'number') {
      return this.ConvertToLetter(EstimationSaving)
    } else {
      return Promise.reject({err: {msg: 'Invalid input!Input must be an array or number.'}})
    }
